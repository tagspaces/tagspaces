  ...
  const handleGeneration = () => {
    ...
    checkActiveProviderModels() // Updated
        ...
        if (success) {
          const promises = generateEntries.map((entry) => {
            return tagsGenerate(entry, fromDescription)
              ...
              .catch((err) => {
                console.error(
                  'Error generating tags for entry:',
                  entry.name,
                  err,
                );
          ...
          Promise.all(promises)
            .then((results) => {
              if (results && results.every((result) => result)) {
                ...
                showNotification(
                  t(
                    'core:aiFailedToGenerateTagsForAll',
                    'Failed to generate tags for any entry.',
                  ),
                );
              }
            })
            .catch((err) => {
              ...
              showNotification(
                t(
                  'core:aiTagGenerationError',
                  'An error occurred during tag generation.',
                ),
              );
            })
            .finally(() => {
      ...
      .catch(() => {
        showNotification(
          t(
            'core:aiProviderCheckFailed',
            'Failed to check AI provider status.',
          ),
        );
        setIsLoading(false);
      });

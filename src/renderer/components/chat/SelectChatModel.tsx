...
export const SelectChatModel = ({
  ...
  const defaultAiProviderFromContext: AIProvider =
    useSelector(getDefaultAIProvider);
  const [installedModels, setModels] = useState<ModelResponse[]>([]);
  const [isLoadingModels, setIsLoadingModels] = useState(false);
  const [customModelPromptDialogOpened, setCustomModelPromptDialogOpened] =
    useState(false);
  ...
  useEffect(() => {
    if (aiProvider) {
      ...
      if (aiProvider.id === defaultAiProviderFromContext?.id) {
        ...
        refreshAIModels(
          aiProvider,
          chosenModel || aiProvider.defaultTextModel,
        ).then(() => {
        ...
        if (aiProvider.engine === 'ollama') {
          specificProviderModelsPromise = getAIClient(aiProvider).then(
            (client) => getOllamaModels(client as Ollama),
          );
        } else if (aiProvider.engine === 'openrouter') {
          specificProviderModelsPromise = getAIClient(aiProvider).then(
            (client) => (client as OpenRouterClient).getOpenRouterModels(),
          );
        } else {
          specificProviderModelsPromise = Promise.resolve([]);
        }
        specificProviderModelsPromise
          .then((fetchedModelsArray) => {
  ...
  function getTitle(model: ModelResponse) {
    ...
    if (model.details) {
      ...
      if (details.parameter_size)
        title += `Parameter Size: ${details.parameter_size}\n`;
      if (details.quantization_level)
        title += `Quantization Level: ${details.quantization_level}\n`;
    }
  ...
  function handleRemoveModel(e) {
    e.stopPropagation();
    if (chosenModel && aiProvider?.engine === 'ollama') {
      // Only for Ollama
  ...
  function handleChangeModel(modelName: string) {
    if (modelName === 'customModel' && aiProvider?.engine === 'ollama') {
      // Only for Ollama
      ...
      if (
        aiProvider &&
        defaultAiProviderFromContext &&
        aiProvider.id === defaultAiProviderFromContext.id
      ) {
    ...
    <>
      ...
      >
        <InputLabel id="model-select-label">
          {aiProvider?.engine === 'ollama' && (
            <OllamaIcon
              width={15}
              style={{ verticalAlign: 'middle', marginRight: '5px' }}
            />
          )}
          {aiProvider?.engine === 'openrouter' && (
            <Typography
              variant="caption"

...
define(function(require, exports) {
  ...
  function initUI() {
    ...
    $('.nav-tabs a[href="#dateRangeTab"]').on('click', function() {
      ...
      $('#dateTimeRangeCalendar').data('DateTimePicker').defaultDate(defaultDateCalendar).viewMode('days').show();
  ...
  function dateCalendarTag(currentDateTime) {
    ...
    if (defaultDateCalendar.toString().length === 7 || defaultDateCalendar.length === 7) {
      ...
      viewMode = 'days';
      format = 'YYYY/MM/DD';
    } else if (currentDateTime.equals(empty)) {
      viewMode = 'days';
      format = 'YYYY/MM/DD';
    }
  ...
  function tagRecognition(dataTag) {
    ...
    if (geoLocationRegExp.exec(currentCoordinate) || geoTag === dataTag) {
      ...
      if (dateTab) {
        ...
        //  $('#dateCalendar').hide();
        //  $('#dateTimeRange').show();
        dateRangeCalendar(currentDateTime);
        //}
      }

...
// import { Grid as GridVirtualized } from 'react-virtualized';
import {
  CellMeasurer,
  CellMeasurerCache,
...
class GridPerspective extends React.Component<Props, State> {
  ...
  componentWillReceiveProps = (nextProps: Props) => {
    ...
    // Directory changed
    if (
      nextProps.currentDirectoryPath !== this.props.currentDirectoryPath // && this.mainGrid
    ) {
    ...
    if (nextProps.directoryContent !== this.props.directoryContent) {
      this.initContent(nextProps.directoryContent);
  ...
  renderCell = (fsEntry: FileSystemEntry) => {
    ...
    if (fsEntry === undefined || (!fsEntry.isFile && !showDirectories)) {
      return;
    }
  ...
  renderMasonry = ({ width }) => {
    ...
    return (
      <Masonry
        autoHeight={true}
        cellCount={this.sortedContent.length}
        ...
        >
          ...
          >
            ...
            <GridVirtualized
              ...
              columnWidth={100}
              height={300}
              rowCount={this.sortedContent.length}
              rowHeight={30}
              width={300}
            /> */}
            {/* <div

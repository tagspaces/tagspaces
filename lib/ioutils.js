...
const { getTabs, getTabId, getOwnerWindow } = require("tabs/utils");
...
exports.getSubdirs = function getSubdirs(dirPath, worker) {
    console.log("Getting subdirectories: "+dirPath);
    try {   
        var dirList = filesIO.list(dirPath);
        var anotatedDirList = [];
        for (var i=0; i < dirList.length; i++) {
        ...
        console.log("Getting subdirectories successful!");     
    } catch(ex) {
...
exports.rename = function rename(filePath, newFilePath, worker) {
	...
	if(!filesIO.exists(newFilePath)) {
        try {
        	...
        	if(runtime.OS.toLowerCase().indexOf("mac")>=0) {
				console.log("Renaming under macos");

				//var url = Cc["@mozilla.org/file/local;1"].newURI(filePath, null, null);
				//if (!url || !url.schemeIs("file")) throw "Expected a file URL.";
				//var binaryFile = url.QueryInterface(Ci.nsIFileURL).file;
				
				var binaryFile = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsILocalFile);
				...
				aFile.initWithPath( newFilePath );
				...
				stream.write(bstream,bstream.length);
